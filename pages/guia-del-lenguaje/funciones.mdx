...
<Callout>
  ...
  que no definen un valor a devolver, devuelven un valor especial de tipo `Void`.
  Esto es, simplemente, una tupla vacía — la cual se escribe como `()`.
</Callout>
...
<Callout>
  ...
  función sin devolver un valor e intentar hacer esto resultará en un error
...
<Callout>
  ...
  opcional, toda la tupla es opcional y no solo cada valor individual dentro
  de la tupla.
</Callout>
...
<Callout>
  El código que escribas como un valor devuelto implícito tiene que devolver algún
  valor. Por ejemplo, no puedes usar `errorFatal("¡Oh, no!")` o `print(13)` como
  valor devuelto implícito.
</Callout>

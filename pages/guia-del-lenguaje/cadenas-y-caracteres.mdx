...
<CodeSnippet>{`let algunaCadena = "Un valor cualquiera para un literal de cadena"`}</CodeSnippet>
...
<CodeSnippet>
  {`let cita = """
El Conejo Blanco se puso sus gafas. «¿Por dónde debo empezar,
...
- Un valor escalar Unicode arbitrario, escrito como `\u{_n_}`, donde _n_ es un número hexadecimal, de uno a ocho dígitos (Unicode se discute en [Unicode](#unicode) abajo).
...
Para indicar si un valor en particular, de tipo `String` puede ser modificado (es decir, que es «mutable») puedes asignarlo a una variable (en cuyo caso, podrá ser modificado), o a una constante (en cuyo caso, no podrá ser modificado):

<CodeSnippet>
  {`var cadenaMutable = "Caballo"\n
cadenaMutable += " y carruaje"
// cadenaMutable es ahora "Caballo y carruaje"\n
let cadenaInmutable = "Highlander"\n
cadenaInmutable += " y otro Highlander"
...
<CodeSnippet>{`let signoDeExclamacion: Character = "!"`}</CodeSnippet>
...
// saludo es ahora igual a "Buenos días"`}
</CodeSnippet>
...
instruccion += cadena2
// instruccion es ahora igual a "deletrea la palabra días"`}
</CodeSnippet>
...
let eTildadaCombinada: Character = "\u{65}\u{301}"          // e seguida de  ́
...
Cada valor de tipo `String` tiene un «tipo de índice» asociado, `String.Index`, el cual corresponde a la posición de cada `Character` en la cadena.
...
<CodeSnippet>
  {`var bienvenida = "Buenas"
bienvenida.insert("!", at: bienvenida.endIndex)
// bienvenida es ahora igual a «Buenas!»\n
bienvenida.insert(contentsOf: " tardes", at: bienvenida.index(before: bienvenida.endIndex))
...
La igualdad de cadenas y caracteres se verifica mediante los operadores «igual que» (`==`) y «no igual que» (`!=`), como se describe en [Operadores de comparación](./operadores-basicos#operadores-de-comparación):
...
<CodeSnippet>
  {`let letraLatinaMayusculaA: Character = "\u{41}"\n
let letraCirilicaMayusculaA: Character = "\u{0410}"\n
if letraLatinaMayusculaA != letraCirilicaMayusculaA {
...
<CodeSnippet>{`let cadenaPez = "Pez‼🐠"`}</CodeSnippet>

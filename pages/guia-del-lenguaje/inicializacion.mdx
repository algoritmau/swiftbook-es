...
<Hero>
  ...
  Set the initial value for a type's stored properties and perform one-time setup.
</Hero>
...
Instances of class types can also implement a _deinitializer_, which performs any custom cleanup just before an instance of that class is deallocated. For more information about deinitializers, see [Deinitialization](./desinicializacion).
...
### Inicializadores de Miembro para Tipos Estructura
...
The rules for how initializer delegation works, and for what forms of delegation are allowed, are different for value types and class types. Value types (structures and enumerations) don’t support inheritance, and so their initializer delegation process is relatively simple, because they can only delegate to another initializer that they provide themselves. Classes, however, can inherit from other classes, as described in [Inheritance](./herencia). This means that classes have additional responsibilities for ensuring that all stored properties they inherit are assigned a suitable value during initialization. These responsibilities are described in [Class Inheritance and Initialization](#class-inheritance-and-initialization) below.
...
<Callout>
  For an alternative way to write this example without defining the `init()` and `init(origin:size:)` initializers yourself, see [Extensions](./extensiones).
</Callout>
...
<DynamicImage
  baseName="initializerDelegation01"
  alt="Flowchart showing the relationships between designated and convenience initializers with their respective rules"
  width={410}
  height={240}
/>
...
<DynamicImage
  baseName="initializerDelegation02"
  alt="Flowchart showing the relationships between designated and convenience initializers with their respective rules"
  width={410}
  height={418}
/>
...
<DynamicImage
  baseName="twoPhaseInitialization01"
  alt="Flowchart showing the two-phase initialization process for a subclass and superclass"
  width={419}
  height={240}
/>
...
<DynamicImage
  baseName="twoPhaseInitialization02"
  alt="Flowchart showing the two-phase initialization process for a subclass and superclass"
  width={419}
  height={240}
/>
...
<Callout>
  Superclass initializers _are_ inherited in certain circumstances, but only when it’s safe and appropriate to do so. For more information, see [Automatic Initializer Inheritance](#automatic-initializer-inheritance) below.
</Callout>


If you want a custom subclass to present one or more of the same initializers as its superclass, you can provide a custom implementation of those initializers within the subclass.

When you write a subclass initializer that matches a superclass _designated initializer_, you are effectively providing an override of that designated initializer. Therefore, you must write the `override` modifier before the subclass’s initializer definition. This is true even if you are overriding an automatically provided default initializer, as described in [Default Initializers](#default-initializers).
...
Conversely, if you write a subclass initializer that matches a superclass _convenience_ initializer, that superclass convenience initializer can never be called directly by your subclass, as per the rules described above in [Initializer Delegation for Class Types](#initializer-delegation-for-class-types). Therefore, your subclass is not (strictly speaking) providing an override of the superclass initializer. As a result, you don’t write the `override` modifier when providing a matching implementation of a superclass convenience initializer.
...
<DynamicImage
  baseName="initializersExample01"
  alt="Initializer chain for the Food class"
  width={410}
  height={144}
/>
...
<DynamicImage
  baseName="initializersExample02"
  alt="Initializer chain for the RecipeIngredient class"
  width={472}
  height={357}
/>
...
<DynamicImage
  baseName="initializersExample03"
  alt="Initializer chain for all three classes"
  width={473}
  height={552}
/>
...
The failable initializer for `CartItem` starts by validating that it has received a `quantity` value of `1` or more. If the `quantity` value isn’t valid, the entire initialization process fails immediately and no further initialization code is executed. Likewise, the failable initializer for `Product` checks the `name` value, and the initialization process fails immediately if `name` is the empty string.
...
<DynamicImage
  baseName="chessBoard"
  alt="A chessboard with alternating black and white squares"
  width={345}
  height={344}
/>

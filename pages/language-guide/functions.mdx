...
<Callout type="note">
  ...
  Names](#function-argument-labels-and-parameter-names) and [Default Parameter
...
As you'll see in [Shorthand Getter Declaration](./properties#shorthand-getter-declaration), property getters can also use an implicit return.

<Callout type="note">
  The code you write as an implicit return value needs to return some value. For
  example, you can't use `fatalError("Oh no!")` or `print(13)` as an implicit
  return value.
</Callout>

...
For more about these capabilities, see [Properties](./properties), [Methods](./methods), [Initialization](./initialization), [Extensions](./extensions), and [Protocols](./protocols).
...
<CodeSnippet>{`var directionToHead = CompassPoint.west`}</CodeSnippet>
...
<CodeSnippet>{`directionToHead = .east`}</CodeSnippet>
...
The syntax used in the examples above marks the enumeration as conforming to the [CaseIterable](https://developer.apple.com/documentation/swift/caseiterable) protocol. For information about protocols, see [Protocols](./protocols).
...
Here, the raw values for an enumeration called `ASCIIControlCharacter` are defined to be of type `Character`, and are set to some of the more common ASCII control characters. `Character` values are described in [Strings and Characters](./strings-and-characters).
...
<Callout type="note">
  ...
  Initializers](../language-reference/declarations#failable-initializers).
</Callout>

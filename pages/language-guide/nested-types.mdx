...
Because `BlackjackCard` is a structure with no custom initializers, it has an implicit memberwise initializer, as described in [Memberwise Initializers for Structure Types](./initialization). You can use this initializer to initialize a new constant called `theAceOfSpades`:

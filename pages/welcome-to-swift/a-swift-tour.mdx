...
<Callout type="experiment">
  Crea una constante con un tipo específico <code>Float</code> y un valor de{' '}
  <code>4</code>.
</Callout>
...
<Callout type="experiment">
  Remueve la conversión a <code>String</code> en la última línea. ¿Cuál es el
  error que aparece?
</Callout>
...
<Callout type="experiment">
  Usa <code>\\()</code> para insertar una operación con números de coma flotante
  en una cadena de texto y para incluir el nombre de una persona en un saludo.
</Callout>
...
<Callout type="experiment">
  Cambia el valor de <code>nombreOpcional</code> a <code>nil</code>. ¿Cuál
  saludo obtienes? Agrega una cláusula <code>else</code> que defina un saludo
  diferente si <code>nombreOpcional</code> es <code>nil</code>.
</Callout>
...
<Callout type="experiment">
  Reemplaza <code>_</code> por una variable con nombre y haz seguimiento del
  tipo de número que resultó ser el mayor.
</Callout>
...
<Callout type="experiment">
  Remueve el parámetro <code>dia</code>. Agrega un parámetro que incluya el
  almuerzo especial de hoy en el saludo.
</Callout>
...
<Callout type="experiment">
  Agrega una constante con <code>let</code> y otro método que tome un argumento.
</Callout>
...
<Callout type="experiment">
  Crea otra subclase de <code>FiguraConNombre</code> llamada{' '}
  <code>Circulo</code>, que tome un radio y un nombre como argumentos de su
  inicializador. Implementa los métodos <code>area()</code> y{' '}
  <code>descripcionBasica()</code> en la clase <code>Circulo</code>.
</Callout>
...
<Callout type="experiment">
  Crea una función que compare dos valores de tipo <code>Escala</code>{' '}
...
<Callout type="experiment">
  Crea un método <code>color()</code> para la enumeración <code>Palo</code> que
  devuelva «negro» para picas y tréboles, y «rojo» para corazones y diamantes.
</Callout>
...
<Callout type="experiment">
  Agrega un tercer caso a <code>RespuestaServidor</code> y al bucle _switch_.
</Callout>
...
<Callout type="experiment">
  Agrega otro requisito a <code>ProtocoloEjemplo</code>. ¿Qué cambios debes
  hacer en <code>ClaseBasica</code> y <code>EstructuraBasica</code> para que
  estas tengan conformidad con el protocolo?
</Callout>
...
<Callout type="experiment">
  Crea una extensión para el tipo <code>Double</code> que agregue la propiedad{' '}
  <code>valorAbsoluto</code>.
</Callout>
...
<Callout type="experiment">
  Cambia el nombre de la impresora a <code>"Nunca Tiene Toner"</code>, de manera
  que la función <code>enviar(tarea:impresoraDestino:)</code> arroje un error.
</Callout>
...
<Callout type="experiment">
  Agrega código de manera que se arroje un error dentro del bloque{' '}
  <code>do</code>. ¿Qué tipo de error es necesario arrojar para que este sea
  manejado por el primer bloque <code>catch</code>? ¿Qué tal con el segundo y
  tercer bloque?
</Callout>
...
<Callout type="experiment">
  Modifica la función <code>anyCommonElements(_:_:)</code>` para crear una
  función que devuelva un _array_ de los elementos que dos secuencias
